...
def main():
    try:
        ...
        try:
            ...
            logger.warning(f"Could not load bike image: {str(e)}")
            st.sidebar.warning("Could not load bike image")

        # Load data if not already loaded
        if not st.session_state.data_loaded:
            hour_df_2012, day_df_summer_2011 = load_data()
            st.session_state.hour_df_2012 = hour_df_2012
            st.session_state.day_df_summer_2011 = day_df_summer_2011
            st.session_state.data_loaded = True
        else:
            hour_df_2012 = st.session_state.hour_df_2012
            day_df_summer_2011 = st.session_state.day_df_summer_2011
